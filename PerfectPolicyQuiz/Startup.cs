...
namespace PerfectPolicyQuiz
{
    public class Startup
    {
        ...
        {
            // add database
            services.AddDbContext<QuizContext>(c => c.UseSqlServer(Configuration.GetConnectionString("QuizConnection")));
            services.AddControllers().AddNewtonsoftJson(x => x.SerializerSettings.ReferenceLoopHandling = Newtonsoft.Json.ReferenceLoopHandling.Ignore);

            //add the Newtonsoft.Json package to allow for disabling the error received when using navigation properties
            services.AddControllers().AddNewtonsoftJson(x => x.SerializerSettings.ReferenceLoopHandling = Newtonsoft.Json.ReferenceLoopHandling.Ignore);
            ...
            {
                opts.RequireHttpsMetadata = false; //only works when it is https
                opts.SaveToken = true;
